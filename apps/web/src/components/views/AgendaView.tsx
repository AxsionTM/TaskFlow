...
export function AgendaView(){
  ...
  const agendaTags=useMemo(()=>{const m=new Map<string,{id:string;name:string;color:string}>();for(const t of dayTasksBase){for(const tt of t.tags||[]){const id=tt.tag?.id||tt.tagId;if(!id||m.has(id))continue;m.set(id,{id,name:tt.tag?.name||"Тег",color:tt.tag?.color||"#888888"});}}return Array.from(m.values());},[dayTasksBase]);
  ...
  return <div className="flex-1 flex flex-col min-h-0">
    ...
    <div className="shrink-0 border-b px-4 py-2.5 space-y-2">
      ...
      {agendaTags.length>0&&<div className="flex flex-wrap gap-1.5">{["",...agendaTags.map(t=>t.id)].map(id=>{const tag=agendaTags.find(t=>t.id===id);const active=id===""?activeTag==="":activeTag===id;return <button key={id||"all"} type="button" onClick={()=>setActiveTag(id)} className={cn("text-xs px-3 py-1.5 rounded-full border transition-all",active?"tf-chip-active":"tf-chip")}>{tag?tagLabel(tag):"Все"}</button>;})}</div>}
    </div>
